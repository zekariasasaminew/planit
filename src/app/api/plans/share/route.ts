...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('share_links')
      .insert({ plan_id: parsed.data.planId, token, is_public: parsed.data.isPublic ?? true, expires_at: parsed.data.expiresAt ?? null })
      .select('token')
      .single();
    if (error) throw error;
    const res = NextResponse.json({ url: `/api/share/${data.token}` }, { status: 201 });
    res.headers.set('X-Request-Id', requestId);
    log.info({ status: 201, elapsedMs: Date.now() - start }, 'POST /api/plans/share');
    return res;
  } catch (e: any) {
