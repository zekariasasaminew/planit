...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: prereqAlts, error: prereqAltsError } = await supabase.from(
    ...
    if (prereqAltsError) {
      log.error(
        { error: prereqAltsError },
        "Failed to fetch prerequisite alternatives"
      );
      throw new Error(
        `Failed to fetch prerequisite alternatives: ${prereqAltsError.message}`
      );
    }

    // Get all courses in alternative groups
    const { data: altCourses, error: altCoursesError } = await supabase.from(
      "course_alternatives"
    ).select(`
        group_id,
        course_id,
    ...
    if (altCoursesError) {
      log.error(
        { error: altCoursesError },
        "Failed to fetch alternative courses"
      );
      throw new Error(
        `Failed to fetch alternative courses: ${altCoursesError.message}`
      );
    }
    ...
    for (const prereq of prereqs || []) {
      if (
        programCourseIds.has(prereq.course_id) &&
        !programCourseIds.has(prereq.prerequisite_course_id)
      ) {
        prereqIds.add(prereq.prerequisite_course_id);
      }
    }
    ...
    for (const groupId of altGroupsWithPrereqs) {
      ...
      if (groupCourses.length > 0) {
        ...
        const firstSequence = groupCourses.filter(
          (c: any) => c.code.includes("151") || c.code.includes("152")
        );
        const selectedSequence =
          firstSequence.length > 0 ? firstSequence : groupCourses.slice(0, 2);
    ...
    if (prereqIds.size > 0) {
      ...
      if (prereqCoursesError) {
        log.error(
          { error: prereqCoursesError, prereqIds: Array.from(prereqIds) },
          "Failed to fetch prerequisite courses"
        );
        throw new Error(
          `Failed to fetch prerequisite courses: ${prereqCoursesError.message}`
        );
      }
      ...
      const res = NextResponse.json(
        {
          ...
          message:
            "No courses found for the selected programs. Please ensure the programs exist and have courses.",
        },
        { status: 400 }
      );
      res.headers.set("X-Request-Id", requestId);
    ...
    nodes = (allCourses || []).map((c) => {
      ...
      for (const prereqAlt of prereqAlts || []) {
        if (prereqAlt.course_id === c.id) {
          ...
          const selectedFromGroup = selectedFromAlternatives.filter(
            (selected: any) => {
              return (altCourses || []).some(
                (ac) =>
    ...
    const res = NextResponse.json(
      { plan, diagnostics, rationale: "Plan generated successfully" },
      { status: 200 }
    );
    res.headers.set("X-Request-Id", requestId);
    log.info(
        ...
        details:
          process.env.NODE_ENV === "development"
            ? {
                ...
                stack: e.stack,
                nodeCount: nodes.length,
                validationResult: parsed?.success
                  ? "valid"
